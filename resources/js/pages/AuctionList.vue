        ...
        <template v-else>
            ...
            <div v-if="stats" class="mb-8 space-y-4">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <div class="bg-white rounded shadow p-4 text-center">
                        <div class="text-3xl font-bold text-purple-600">{{ stats.total_items }}</div>
                        <div class="text-xs text-gray-500 mt-1">Items Up for Grabs</div>
                    </div>
                    <div class="bg-white rounded shadow p-4 text-center">
                ...
                <div class="grid gap-4 sm:grid-cols-3">
                    ...
                    <div class="bg-white rounded shadow p-4">
                        <h3 class="text-sm font-semibold text-gray-700 mb-3">Bidding Activity (7 days)</h3>
                        <div class="flex items-end gap-1 h-24">
                            <div v-for="day in stats.bids_per_day" :key="day.date"
                                class="flex-1 flex flex-col items-center gap-1">
                                <span v-if="day.count > 0" class="text-xs text-gray-500">{{ day.count }}</span>
                                <div class="w-full bg-blue-400 rounded-t transition-all"
                                    :style="{ height: (day.count / bidsMax * 100) + '%', minHeight: day.count > 0 ? '4px' : '2px' }"
                                    :class="day.count === 0 ? 'bg-gray-200' : 'bg-blue-400'">
                                </div>
                    ...
                    <!-- Hot auctions -->
                    <div class="bg-white rounded shadow p-4">
                        <h3 class="text-sm font-semibold text-gray-700 mb-3">Most Competitive</h3>
                        <div v-if="stats.hot_auctions.length === 0" class="text-xs text-gray-400">No active auctions</div>
                        <ul v-else class="space-y-2">
                            <li v-for="a in stats.hot_auctions" :key="a.id">
                                <router-link :to="`/auctions/${a.id}`" class="flex items-center justify-between hover:bg-gray-50 rounded px-1 -mx-1">
                                    <span class="text-sm truncate">{{ a.title }}</span>
                                    <span class="text-xs font-medium text-blue-600 ml-2 shrink-0">{{ a.bid_count }} bids</span>
                                </router-link>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="bg-white rounded shadow p-4 space-y-4">
                        <div>
                            <h3 class="text-sm font-semibold text-gray-700 mb-2">Top Bidders</h3>
                            <div v-if="stats.top_bidders.length === 0" class="text-xs text-gray-400">No bids yet</div>
                            <div v-else class="space-y-1">
                                <div v-for="(b, i) in stats.top_bidders" :key="b.username" class="flex items-center gap-2">
                                    <span class="text-xs font-bold w-4" :class="[
                                        i === 0 ? 'text-amber-500' : i === 1 ? 'text-gray-400' : i === 2 ? 'text-amber-700' : 'text-gray-300'
                                    ]">{{ i + 1 }}</span>
                                    <span class="text-sm">{{ b.username }}</span>
                                    <span class="text-xs text-gray-400 ml-auto">{{ b.auction_count }} auctions</span>
                                </div>
                            </div>
                        </div>
                        <div v-if="stats.ending_soon.length > 0">
                            <h3 class="text-sm font-semibold text-red-600 mb-2">Ending Soon</h3>
                            <ul class="space-y-1">
                                <li v-for="a in stats.ending_soon" :key="a.id">
                                    <router-link :to="`/auctions/${a.id}`" class="text-sm text-red-600 hover:underline">
                                        {{ a.title }} — {{ timeLeft(a.ends_at) }}
                                    </router-link>
                                </li>
                            </ul>
                        </div>
            ...
            <p v-if="auctions.length === 0" class="text-gray-500">No auctions yet.</p>
            <div v-else class="grid gap-4 sm:grid-cols-2">
                ...
                >
                    <img v-if="auction.images.length" :src="auction.images[0].url" :alt="auction.title"
                    ...
                    <div class="mt-1 text-xs text-gray-400">
                        <span v-if="auction.quantity > 1" class="text-purple-600 font-medium">{{ auction.quantity }} items · </span>
                        {{ auction.bid_count }} bid{{ auction.bid_count !== 1 ? 's' : '' }} · by {{ auction.seller.username }}
                    </div>
                    </div>
                </router-link>
            </div>
