...
<body>
    ...
    <table cellpadding="0" cellspacing="0" style="background-color: #1e3e50; color: #ffffff;">
        <tr>
            ...
            <td style="width: 25%; padding: 22px 15px; vertical-align: top; font-size: 8.5px; line-height: 1.6;">
                <span style="font-size: 9px; font-weight: bold;">{{ __('quote.visiting_address') }}</span><br>
                {{ $company['name'] }}<br>
                {{ $company['street'] }}<br>
                {{ $company['postal_code'] }} {{ $company['city'] }}
            </td>
            <td style="width: 25%; padding: 22px 15px; vertical-align: top; font-size: 8.5px; line-height: 1.6;">
                <span style="font-size: 9px; font-weight: bold;">{{ __('quote.business') }}</span><br>
                {{ __('quote.kvk_label') }} {{ $company['kvk'] }}<br>
                {{ __('quote.btw_label') }} {{ $company['btw'] }}
            </td>
    ...
    <div style="padding: 40px 50px 120px 50px;">
        ...
        <table style="margin-bottom: 50px;">
            <tr>
                <td style="width: 180px; padding: 3px 0; font-size: 10px;">{{ __('quote.date') }}</td>
                <td style="padding: 3px 0; font-size: 10px;">{{ $generated_at }}</td>
            </tr>
            @if(count($items) === 1)
            <tr>
                <td style="padding: 3px 0; font-size: 10px;">{{ __('quote.item') }}</td>
        ...
        <table>
            ...
            <tr>
                ...
                <td style="padding: 4px 0; font-size: 10px; text-align: right; padding-right: 15px;">
                    <strong>{{ $btw_percentage }}% {{ __('quote.btw_label') }}</strong>
                    &nbsp;&nbsp;<span style="color: #666666;">{{ __('quote.over') }} {{ $currency }} {{ number_format($subtotal, 2, ',', '.') }}</span>
                </td>
        ...
        <div style="margin-top: 10px; font-size: 10px; line-height: 1.6;">
            {!! __('quote.payment_reference_note', ['reference' => '<strong>' . e($payment_reference) . '</strong>']) !!}
        </div>
        @endif
    ...
    <div style="position: fixed; bottom: 0; left: 0; right: 0; border-top: 1px solid #cccccc; padding: 15px 50px;">
        <table>
            <tr>
                ...
                <td style="text-align: right; font-size: 9px;">{{ __('quote.page') }}</td>
            </tr>
        </table>
    </div>
